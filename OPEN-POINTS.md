...
| **Qwen3-4B-Instruct Q4_0 (local gguf, unsloth)** | **Q4_0** | Qwen3 | **PASS (step 1, loss 7.538)** |

**RESOLVED 2026-06-17: a 4B voice is shippable today, no waiting on QVAC.** The SDK team (Gianfranco) confirmed the failures are expected (unsupported quant), that they can add Q4_0/Q8_0 4B/8B constants to the registry later, and that `QWEN3_4B_INST_Q4_SHARD` is indeed Q4_K_M. Crucially: **you can pass your own local GGUF** via `modelSrc` (a plain path string, per the SDK's own `examples/llamacpp-filesystem.js`, and proven above). So I downloaded a genuine **Q4_0 Qwen3-4B-Instruct** (unsloth, 2.2 GB, `https://huggingface.co/unsloth/Qwen3-4B-Instruct-2507-GGUF/resolve/main/Qwen3-4B-Instruct-2507-Q4_0.gguf`), ran `loadModel({ modelSrc: "/path/to/it.gguf", modelType: "llm" })` then `finetune({...})`, and it **trained** (first step, loss 7.538, on the real 1105-row dataset). To wire a 4B voice: add a base in `spike/finetune.js` BASES + `app/lib/models.js` BASES that points at a local Q4_0 gguf path (not an SDK constant), add it to `catalog.js` `CURATED`, and let the app download/manage the gguf. Estimated cost (M5 Max 36 GB): ~4-5 h train, ~5-7 GB RAM (re-verify the no-intermediate-checkpoint hang fix at 4B before an unattended run). Llama-architecture support is not coming and not needed (SDK team: "old arch", they're adding newer ones alongside qwen3/gemma3/bitnet).
